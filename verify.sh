...
echo -n ".dev MCP server…" && curl -s https://workswithagents.dev/ | grep -qi "MCP Server" && echo " ✅" || echo " ❌"
echo -n ".dev 12 tools…" && curl -s https://workswithagents.dev/ | grep -qi "12 tools" && echo " ✅" || echo " ❌"
echo -n ".dev tools.json…" && curl -s https://workswithagents.dev/tools.json | python3 -c "import sys,json; d=json.load(sys.stdin); assert len(d['tools'])==12" && echo " ✅" || echo " ❌"
echo -n ".dev llms.txt…" && curl -s https://workswithagents.dev/llms.txt | grep -q "bidcheck" && echo " ✅" || echo " ❌"
echo -n ".dev no specs/…" && curl -s https://workswithagents.dev/ | grep -vq "/specs/" && echo " ✅" || echo " ❌"
echo -n ".dev no cdn-cgi…" && curl -s https://workswithagents.dev/ | grep -vq "cdn-cgi" && echo " ✅" || echo " ❌"
...
echo -n "digital sector……" && curl -s https://workswithagents.com/sectors/digital-it/ | grep -qi "cyber essentials" && echo " ✅" || echo " ❌"
